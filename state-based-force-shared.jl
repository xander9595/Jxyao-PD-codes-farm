...
@everywhere function dilatation_kernal!(dilatation, m, influence_state, ref_mag_state, exten_state, families, irange, jrange)
    for i in irange
        if m[i,1] != 0
...
@everywhere dilatation_kernal_shared!(dilatation, m, influence_state, ref_mag_state, exten_state, families) =
                dilatation_kernal!(dilatation, m, influence_state, ref_mag_state, exten_state, families, myrange(families)...)
...
function dilatation_shared!(dilatation, m, influence_state, ref_mag_state, exten_state, families)
    @sync begin
        for p in procs(families)
            @async remotecall_wait(dilatation_kernal_shared!, p, dilatation, m, influence_state, ref_mag_state, exten_state, families)
        end
    end
end
...
Profile.clear()
@profile compute_inrenal_force_shared(Coords , Disp , families, 3.015*width_x/ndivx, mask)
ProfileView.view()
@profview compute_inrenal_force_shared(Coords , Disp , families, 3.015*width_x/ndivx, mask)
